#!/usr/bin/env python3
"""Extract CTR's BIGFILE.BIG code modules as recompiler overlay images.

CTR keeps its overlays inside one 270 MB archive rather than as separate disc files, so the shipping
emitter's `--overlays DIR` input has to be produced. The archive index is a table of
(sector offset, byte size) pairs starting at word 2 of the file; entry N's bytes are the module the
game's loader reads to a measured base (see `game/recomp_seeds.json` `overlay_bases`).

The stems in that seed file are the authority for WHICH entries matter: `BF<id>` names archive entry
...
by name, because a silently empty overlay directory would emit a product that fails fast much later
with an unexplained recomp-MISS.
"""
...
def seed_stems(path: Path = SEEDS) -> dict[str, int]:
    """Map each `overlay_bases` stem to its archive entry id."""
    text = path.read_text(encoding="utf-8")
    stripped = re.sub(r"^\s*//.*$", "", text, flags=re.MULTILINE)
    bases = json.loads(stripped).get("overlay_bases", {})
    stems: dict[str, int] = {}
    for stem in bases:
        ...
        stems[stem] = int(matched.group(1), 10)
    return stems
...
def extract(disc_argument: str | None, discdump_override: str | None) -> int:
    stems = seed_stems()
    if not stems:
        print("[overlays] the seed file declares no overlay_bases; nothing to extract")
        return 0
    ...
    with archive.open("rb") as handle:
        ...
        print(
            f"[overlays] {BIGFILE_NAME} index: {len(entries)} entries, {len(stems)} named by the seed file"
        )
...
def selftest() -> None:
    ...
    print(
        f"[overlays-selftest] PASS {checks}/{checks} checks: index parsing, refusals, and both classifier verdicts"
    )
