...
`0x8003CEB4`. The driver preserves the ordinary retail bodies for the three startup/teardown predecessors
...
crosses startup; later host steps resume at `0x8003CEB4`. Each finite step ends when retail
...
bridge register effects. The shipping product reaches those original retail bodies through its executor and
...
and CD service which can make the poll advance. The title driver now preserves the retail setup,
...
that original retail body. The grounded safe seam is startup loop `0x8003C94C`: its wrapper first lets
the custom-exit poll finish and clear `in_irq`, then CTR delivers DMA4 before the retail body's
...
VSync(2) branch, not a new policy. The title owner admits that measured continuation; other
...
aborted because it treated all three admitted suffixes as non-returning. Retail binary code
grounds the distinction: `0x800336F8` is inside `0x80033610`, and that function's sole direct caller
resumes at `0x8003CC98`. The title owner requires that exact reentry before dispatching the caller
continuation. A production transition contract
...
Serialized PID 3229162 crossed the newly admitted `0x8003CC98` path and next reached timing leaf
...
retail function `0x8004B41C` is its sole other direct caller, uses the result as an elapsed-time query, and
...
Independent differential execution evidence still stops at `0x800772E0`. The title driver owns the
exact fifth-argument-`-1` prefix, yields two native fields, and resumes through measured entries
`0x80032074`, `0x8003C8D4`, and `0x8003C984`; that recorded route remains unverified. The
native/Lightrec product must reach or falsify it.
...
For each reachable caller, extract the smallest owner which preserves the retail prefix/suffix,
models its finite native wait, and resumes through the measured continuation. Do not replace VSync
...
### Note (2026-08-28)
Serialized evidence advanced the frame owner twice and exposed a title adapter omission at scoped
VSyncCallback registration `0x80077254`. The current owner preserves that retail call through the
runtime's scoped-original boundary and requires the exact measured address.
...
Resolution boundary: preserve the measured native owners and fatal VSync trap while issue 0024
proves the current Lightrec and typed-exit composition. Resolve the remaining reachable VSync set from the dynamic product's first
