...
- status: re-partial
- deps: CTR-03
- evidence: C004/I004, C005/I005, C006/I006, C007/I007, C009/I008, C010/I009, C011/I010, and C012/I011. `tools/emit_substrate.py` re-verified the complete USA executable identity before invoking the shipping emitter. With an empty explicit seed manifest, the executable-header entry plus direct-call discovery emitted 1,236 functions in eight shards (recompiler version 2026-08-12.1). `ctr04_check` preserves the independent pre-BIOS proof at 34/34 fields. `ctr04_post_init_heap_check` ran the independent CPU twice with identical state/step evidence, verified the observed A(39h) vector and applied the framework's explicit `v0=0` leaf contract, then compared initial-call, modeled-return, and first-subsequent-call state against generated execution: 108/108 fields agreed at `0x8003C58C`; forced `post.gp=0` produced 107/108. Independent Ghidra disassembly of original `0x8003C58C..0x8003C5AC` proved a store-only prefix before its first call. `ctr04_resident_next_call_check` validates those exact bytes, repeats both original-state and bounded-replay oracle runs, and uses canonical `--capture-call 1` to discover `0x800779E4`; generated execution agreed 34/34 there and forced `resident.gp=0` produced 33/34. Ghidra then proved `0x800779E4` is a one-time runtime initializer whose constructor count is zero and whose two data inputs remain unchanged initialized words before BSS. `ctr04_runtime_init_next_call_check` validates that function, its caller continuation, both data words, and both stack-write spans; two `--capture-call 2` oracle replays were identical and generated execution agreed 34/34 at `0x80032DC0`, while forced `resident.gp=0` produced 33/34. Ghidra proved `0x80032DC0`'s startup idle path reads only request word 1, BSS loading flag 0, request count 0, and timestamp 0 before returning. `ctr04_startup_service_next_call_check` validates those three words and four exact instruction islands, reserves all evidence ranges from trampoline placement, and agrees 34/34 at `0x8001D06C`; forced `resident.gp=0` produces 33/34. Ghidra and exact words then prove that callee returns on BSS-zero pending work, mode zero selects jump-table entry `0x8003C614`, and the first initialization call is executable A(2Bh) thunk `0x800718BC`. `ctr04_startup_memset_thunk_check` checks both callee islands, the complete dispatch, callsite, table word, state pointer, and BSS input; repeated call-ordinal-4 oracle replay agrees 34/34 with generated execution, while forced `resident.gp=0` produces 33/34.
- where: `game/recomp_seeds.json`, `game/core/crt0_port_trace.cpp`, `tools/emit_substrate.py`, `tools/compare_crt0_trace.py`, `tools/resident_replay.py`, CMake `ctr04_check`/`ctr04_post_init_heap_check`/`ctr04_resident_next_call_check`/`ctr04_runtime_init_next_call_check`/`ctr04_startup_service_next_call_check`/`ctr04_startup_memset_thunk_check`/`ctr04_startup_post_memset_next_call_check`/`ctr04_startup_init_swap_next_call_check`; gitignored `generated/`, `scratch/raw/ctr/ctr04-resident-replay.exe`, and boundary traces
- gap: The verified window stops at the ENTRY of executable dispatch thunk `0x800771C4`, reached past the explicitly modeled A(2Bh) memset (destination `[0x80096B20,+0x2584)` poisoned on both CPUs, checked thunk redirected to a byte-loop model), the caller jal island `0x8003C62C..0x8003C630`, and the whole executable swap function `0x80077CD8` (delay-slot store into initialized word `0x8008C0B4`, previous value returned and ignored). The next call is INDIRECT: instruction `0x800771DC` loads target `0x800772E0` from slot `0x8008C00C` through the table pointer stored at `0x8008C020` (value `0x8008C000`) and executes `jalr`; the target is the one-time system initializer (Ghidra `FUN_800772e0`, guards all work on word `0x8008AF98`). Crossing it is blocked on two named facts, measured 2026-08-24: (1) framework `oracle_trace --capture-call N` counts only executed direct `jal` boundaries, so the jalr target can be neither captured nor counted — the ordinal-7 attempt ran past `0x800772E0`'s entry into its body and refused at instruction `0x80077334`, which writes `0x33333333` through the initialized pointer stored at `0x8008C02C` (value `0x1F8010F0`, DPCR), with denominator "reached 6 of 7"; a validated capture/stop-at-pc mode is the proper framework fix. (2) Generated execution of the jalr refuses routing because the consumer runtime supplies no `GuestProgramImage` (`overlay_router.cpp` refuses when the image or its `residentText` range is absent; the fix is consumer-side: fill CtrRuntime's image fact with the already-measured resident text `[0x80010000,0x8008D800)`). Tool-level support exists behind `--startup-init-dispatch-next-call` (dispatcher-prefix bytes plus both dispatch words are checked constants) and refuses honestly until both facts land. Do not treat the initializer's body as executed evidence; it programs device registers directly.
- notes: The generated tracer takes interception targets from canonical oracle output and independently requires them in the generated registry. The resident replay is not general continuation: each captured boundary is valid only because every traversed code range, executable-backed data input, and stack-write span is mechanically checked and excluded from trampoline placement. External leaves with RAM effects are boundaries, not register-only returns; the A(2Bh) memset is the first modeled RAM-mutating leaf, and its model is proven by destination poisoning — a missing or wrong write cannot pass silently. The emitter reported 28 unresolved `lw $ra` bases, so the 1,236 emitted entries are substrate inventory, not a claim that every unexecuted return edge is resolved. All three functions on this stretch (`0x80077CD8`, `0x800771C4`, `0x800772E0`) are present in the emitted registry; presence is not execution proof, which each window supplies separately.
