...
except that old static-recompiler guidance is superseded by
`../../shared/jit-common/docs/migration.md`.
...
- An interpreter may be built only in a separate test/diagnostic target. Product link and selector
  checks must prove it is absent and that unsupported execution cannot fall back to it.
...
- Do not regenerate, build, or run the static product. Its recorded boundary and binary facts remain
  migration evidence until representative gameplay passes the deletion gate in `docs/migration.md`.

## Explicit executor exits

`game/core/frame_driver.cpp` currently throws and catches local `FrameCompleted` to escape generated
host frames from wait, service, and frame-completion owners. That mechanism cannot cross JIT frames.
Replace it with a typed psxport executor-exit request: the native callback records its exact title
continuation/state and asks the executor to stop; Lightrec returns normally at a safe dispatcher
boundary; `CtrFrameDriver::stepFrame` handles the result, restores the captured diagnostic depth,
finishes one field/presentation fence, and updates counters. Unexpected normal return or an
unrecognized exit remains fatal. Do not translate the exception into `longjmp` or another host-stack
escape.
...
active. It does not authorize static deletion.
