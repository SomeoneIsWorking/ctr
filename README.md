...
CCACHE_DISABLE=1 cmake --build build --target ctr_scaffold
```

`ctr_scaffold` only proves that the game-agnostic framework links. It does not launch Crash Team
Racing. See `titles/ctr/README.md` for the measured target and `docs/re-frontier.md` for the ordered
work required before a boot claim is possible.
